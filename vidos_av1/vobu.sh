...
checkOpts() {
	if [ -z $1 ] || [ "-" = $1 ] ; then echo "invalid option"
	        ARG_VALID=1
	fi
}
...
mkdir -p $DIR/initramfs_overlay/firmware
...
do
	...
	do
			...
			cp -r $DIR/firmware/$FIRMWARES -t $DIR/initramfs_overlay/firmware/
	        fi
	done
...
rm -r initramfs_overlay/firmware/
if [ -n "$(ls $VIDOS_ISO9660/video/)" ]; then rm $VIDOS_ISO9660/video/*
elif [ -n "$(ls initramfs_overlay/opt/)" ]; then rm initramfs_overlay/opt/*
fi
